...
write_status() {
  ...
  jq -n \
    ...
    --arg note "$note" \
    '{state:$state, profile:$profile, collector:$collector, generated_at:$generated_at, note:$note}' > "$TMP_DIR/time_${PROFILE_ID}_status.json.$$"
  mv -f "$TMP_DIR/time_${PROFILE_ID}_status.json.$$" "$STATUS_JSON"
}
