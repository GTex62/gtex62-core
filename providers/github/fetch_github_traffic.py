    ...
    for item in repos:
        if isinstance(item, str) and item.strip():
            ...
            if repo:
                ...
                cl = item.get("complete_lifetime")
                cl = bool(cl) if isinstance(cl, bool) else None
                out.append({"repo": repo, "note": note, "complete_lifetime": cl})
    return out
...
def main():
    ...
    for item in repos:
        repo = item["repo"]
        note = item.get("note")
        complete_lifetime_override = item.get("complete_lifetime")
        try:
