...
write_status() {
  jq -n \
    ...
    --arg note "$2" \
    '{state:$state, profile:$profile, collector:$collector, generated_at:$generated_at, note:$note}' > "$TMP_DIR/network_${PROFILE_ID}_status.json.$$"
  mv -f "$TMP_DIR/network_${PROFILE_ID}_status.json.$$" "$STATUS_JSON"
}
